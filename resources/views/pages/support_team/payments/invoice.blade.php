                                    ...
                                    <td style="min-width:220px;">
                                        ...
                                        <select id="months-select-{{ $hash }}" class="form-control months-select"
                                            ...
                                            @foreach ($monthsList as $index => $m)
                                                ...
                                                <option value="{{ $m }}"
                                                    @if ($paid_months >= $index + 1) disabled style="background:#eee;color:#666;" @endif>
                                            ...
                                            <form action="{{ route('payments.pay_now', Qs::hash($uc->payment->id)) }}"
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <small class="text-muted">You can pay any amount up to
                                                        {{ number_format($uc->payment->additional_amount, 2) }}
                                                        LKR.</small>
                                                </div>
                                        ...
                                        <div class="list-icons">
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-left">
                                                    <a id="{{ $hash }}" onclick="confirmReset(this.id)"
                    ...
                    (function() {
                        ...
                        document.querySelectorAll('.ajax-pay').forEach(form => {
                            form.addEventListener('submit', function(e) {
                                ...
                                const totalInput = document.querySelector(`.total-amount-input`);
                                console.log("ddd", totalInput);

                                const totalValue = parseFloat(totalInput?.value || 0);
                                console.log("ddd", totalValue);

                                // Create a FormData object from your form
                                ...
                                fetch(form.action, {
                                    ...
                                    .then(async res => {
                                        ...
                                        if (!res.ok) {
                                            ...
                                            if (res.status === 401 || res.status === 403) return showError(
                                                'Not authorized.');
                                            if (res.status === 422) return showError(data.message ||
                                                'Validation failed.');
                                        ...
                                        if (data.ok) {
                                            ...
                                            const paidCell = form.closest('tr').querySelector(
                                                'td:nth-child(5)');
                                            if (paidCell) {
                                            ...
                                            const pendingCell = form.closest('tr').querySelector(
                                                '.pending-amount');
                                            if (pendingCell) pendingCell.textContent = remainder.toFixed(2);

                                            const selectedMonths = Array.from(select.selectedOptions).map(
                                                o => o.value);
                                            Array.from(select.options).forEach(opt => {
